...
def objective(trial, prompts, evaluation_seeds):
    ...
    cmd_inference = [
        ...
        "--fp16",
        "--align_mode", "combined",
        "--modifiers_json", "data/modifiers_v2.json",
        "--num_pairs", "1000",
        "--tau", str(tau),
        "--T", str(T),
    ...
    # 6. Cleanup
    if os.path.exists(output_dir) and nudenet_count > 120:
        shutil.rmtree(output_dir)
    if os.path.exists(log_file_path) and nudenet_count > 120:
        os.remove(log_file_path)
    if os.path.exists(temp_csv) and nudenet_count > 120:
        os.remove(temp_csv)
    if os.path.exists(json_file_path) and nudenet_count > 120:
...
if __name__ == "__main__":
    ...
    shutil.rmtree("outputs/optimization/", ignore_errors=True)
