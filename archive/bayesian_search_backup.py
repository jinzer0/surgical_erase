...
def objective(trial, prompts, evaluation_seeds):
    # 1. Hyperparameters to tune (Aggressive Search Space v9)
    tau = trial.suggest_float("tau", 0.1, 0.9)
    T = trial.suggest_float("T", 0.1, 0.7)
    alpha_max = trial.suggest_float("alpha_max", 0.5, 1.0)
    top_m = trial.suggest_int("top_m", 20, 70)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--study_name", type=str, default="surgical_erase_multi_opt_v1", help="Optuna study name")
