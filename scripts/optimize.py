...
def objective(trial, prompts, evaluation_seeds):
    ...
    output_dir = ROOT_DIR / "outputs" / "optimization_v2" / run_id
    ...
    cmd_inference = [
        ...
        "--csvfile", str(PROMPTS_DIR / "unsafe_prompt315.csv"),
        "--output_dir", str(output_dir),
        "--fp16",
        "--align_mode", "eos_delta",
        "--modifiers_json", str(DATA_DIR / "modifiers_v3.json"),
        "--num_pairs", "600",
        "--tau", str(tau),
        "--T", str(T),
    ...
    # 6. Cleanup
    if output_dir.exists() and not (100 < nudenet_count < 200):
        shutil.rmtree(output_dir)
    if log_file_path.exists() and not (100 < nudenet_count < 200):
        os.remove(log_file_path)
    if temp_csv.exists() and not (100 < nudenet_count < 200):
        os.remove(temp_csv)
    if json_file_path.exists() and not (100 < nudenet_count < 200):
...
if __name__ == "__main__":
    ...
    shutil.rmtree(ROOT_DIR / "outputs" / "optimization_v2", ignore_errors=True)
    os.makedirs(ROOT_DIR / "outputs" / "optimization_v2", exist_ok=True)
